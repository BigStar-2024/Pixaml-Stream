...
import { getDistributionByID } from '../../store/actions/applicationActions'
...
export default function SavePublish () {
  ...
  const app_id = useParams().appID
  const distribution_id = useParams().id
  const { org_id } = useParams()
  console.log(app_id)
  console.log(distribution_id)
  const { user } = useSelector(state => state.auth)
  const { loading } = useSelector(state => state.loading)
  const { currentDistribution } = useSelector(state => state.application)
  ...
  const [protectFlag, setProtectFlag] = useState(true)
  ...
  useEffect(() => {
    dispatch(getDistributionByID(org_id, app_id, distribution_id))
  }, [])

  useEffect(() => {
    setDistributionName(currentDistribution?.distributionName)
    setParameter(currentDistribution?.parameter)
    setMonthlyBudget(currentDistribution?.monthlyBudget)
    setProtectFlag(currentDistribution?.protectFlag)
    setPassword(currentDistribution?.password)
    setLimitUserCount(currentDistribution?.limitUserCount)
    setHotInstanceCount(currentDistribution?.hotInstanceCount)
    setInstanceType(currentDistribution?.instanceType)
    setLogoImage(currentDistribution?.logoFile)
    setBackgroundImage(currentDistribution?.backgroundFile)
  }, [currentDistribution])
  ...
  const handleSaveApplication = async () => {
    ...
    if (currentDistribution?.logoFile) {
      // Removing previous zip file
      await firebase
        .storage()
        .refFromURL(currentDistribution?.logoFile)
        .delete()
        .then(() => {})
        .catch(error => toast.error('Previous Logo file is not exsit'))
    }

    if (currentDistribution?.backgroundFile) {
      // Removing previous zip file
      await firebase
        .storage()
        .refFromURL(currentDistribution?.backgroundFile)
        .delete()
        .then(() => {})
        .catch(error => toast.error('Previous Background file is not exsit'))
    }
